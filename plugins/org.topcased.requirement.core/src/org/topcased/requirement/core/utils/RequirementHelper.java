...
public final class RequirementHelper {
	...
	/**
	 * Constant used to indicate that an anonymous requirement creation is
	 * expected
	 */
			...
			if (extension != null) {
				...
				if (REQUIREMENT_FILE_EXTENSION.equals(extension)) {
					if (!modelResource.getContents().isEmpty()
							&& modelResource.getContents().get(0) instanceof RequirementProject) {
						result = (RequirementProject) modelResource
								.getContents().get(0);
					}
				} else if (extension.endsWith("di")) //$NON-NLS-1$
				{
					if (modelResource.getContents().get(0) instanceof Diagrams) {
						result = getRequirementProject((Diagrams) modelResource
								.getContents().get(0));
					}
				} else {
					URI uri = URI.createURI(modelResource.getURI().toString()
							+ "di"); //$NON-NLS-1$
					Resource diResource = modelResource.getResourceSet()
							.getResource(uri, true);
					if (diResource != null
							&& diResource.getContents().get(0) instanceof Diagrams) {
						result = getRequirementProject((Diagrams) diResource
								.getContents().get(0));
					}
				}
			}
	...
	public RequirementProject getRequirementProject(Diagrams diagrams) {
		RequirementProject result = null;
		if (diagrams != null && diagrams.eResource() != null
				&& diagrams.eResource().getResourceSet() != null) {
			IModelAttachmentPolicy policy = ModelAttachmentPolicyManager
					.getInstance().getModelPolicy(
							diagrams.eResource().getResourceSet());
			if (policy == null) {
				policy = DefaultAttachmentPolicy.getInstance();
			}
			if (policy != null) {
				result = policy
						.getRequirementProjectFromTargetDiagram(diagrams);
			}
		}
		return result;
	}
	...
	/**
	 * Called from the modeler. Allows to create current requirements (by
	 * drag'n'drop or directly from the contextual menu).
	 * 
	 * @param droppedObjects
	 *            The list of dropped object.
	 * @param targetObject
	 *            The target object
	 */
	public Command createCurrentRequirements(Collection<?> droppedObjects,
			EObject targetObject) {
		return executeRequirementCreation(droppedObjects, targetObject,
				CREATE_CURRENT);
	}
	...
	public Command createCurrentRequirement(EObject targetObject) {
		return executeRequirementCreation(Collections.emptyList(),
				targetObject, CREATE_CURRENT);
	}
	 ...
	 * 
	 * @param droppedObjects
	 *            The list of dropped object in case of a drag'n'drop.
	 * @param targetObject
	 ...
	 * @return
	 */
	private Command executeRequirementCreation(Collection<?> droppedObjects,
			EObject targetObject, boolean kind) {
		...
		if (targetObject != null) {
			...
			HierarchicalElement hierarchicalElement = getHierarchicalElement(
					targetObject, globalCmd);

			if (kind == CREATE_CURRENT) {
				createdRequirements = addCommandRequirement(droppedObjects,
						hierarchicalElement, globalCmd);
			} else {
			...
			currentPage.setSelection(new StructuredSelection(
					createdRequirements));
	 ...
	 * @param create
	 */
	public static void appendIfCanExecute(CompoundCommand globalCmd,
			Resource toCheck,EditingDomain domain, Command com) {
	 ...
	 * If the corresponding hierarchical element does not already exist, it is
	 * created and returned.<br>
	 * Note that all the hierarchy is implicitly created when a new element is
	 * created.
	 * 
	 * @param targetObject
	 *            The dropped target model object
	 * @param globalCmd
	 *            The compound command in which other commands must be added.
	 * @return The {@link HierarchicalElement} corresponding to the targeted
	 *         model object.
	 */
	public HierarchicalElement getHierarchicalElement(EObject targetObject,
			CompoundCommand globalCmd) {
		// Find the hierarchical element corresponding to the targetObject in
		// the model
		HierarchicalElement hierarchicalElement = RequirementUtils
				.getHierarchicalElementFor(targetObject);
	 ...
	 * If the targetObject does not exists, the hierarchical element is created
	 * and search the parent of this new element.<br>
	 * If the targetObject object exists, add the EObject as a child to this
	 * element.
	 * 
	 * @param parent
	 ...
	 * @param cmd
	 *            A compound command in which EMF command must be stacked.
	 * @return the hierarchical element found or just created
	 */
	private HierarchicalElement attach(EObject parent, EObject child,
			CompoundCommand cmd) {
	...
	/**
	 * A simple method to get the policy and check if the current element is the
	 * root
	 * 
	 * @param parent
	 *            the current element
	...
	private boolean isElementRoot(EObject parent) {
		...
		if (modeler != null) {
			IModelAttachmentPolicy policy = ModelAttachmentPolicyManager
					.getInstance().getModelPolicy(modeler.getEditingDomain());

			if (policy != null) {
				return policy.isRootContainer(parent);
			} else {
				return DefaultAttachmentPolicy.getInstance().isRootContainer(
						parent);
			}
		}
		return false;
	 ...
	 * 
	 * @param modelElement
	 *            A model element referenced by the hierarchical element to
	 *            create.
	 * @return the hierarchical element created
	 */
	private HierarchicalElement createHierarchicalElement(EObject modelElement) {
		HierarchicalElement newSystem = RequirementFactory.eINSTANCE
	...
	/**
	 * Executes the command to create the current requirements with the dropped
	 * requirements (upstream or a current requirements)
	 * 
	 * @param droppedObjects
	 *            A collection of requirements (upstream or current)
	 * @param target
	 *            The hierarchical element where to create the current
	 *            requirements
	 * @return a list of Requirements created
	 */
	private List<org.topcased.requirement.Requirement> addCommandRequirement(
			Collection<?> droppedObjects, EObject target,
		...
		if (target != null) {
			...
			if (droppedObjects.isEmpty()) {
				CurrentRequirement toCreate = create(hierarchicalElt, null,
						compoundCmd);
				toSelect.add(toCreate);
			} else {
				for (Object object : droppedObjects) {
					Integer pos = AddRequirementMarker.eINSTANCE
							.computeIndex(target);
					org.topcased.requirement.Requirement toCreate = null;
					if (object instanceof Requirement) {
						// from the upstream view...
						toCreate = create(hierarchicalElt,
								(Requirement) object, compoundCmd);
						toSelect.add(toCreate);
						...
						if (!target.equals(requirement.eContainer())) {
							Command dndCmd = DragAndDropCommand.create(
	...
	/**
	 * Creates a <b>{@link CurrentRequirement}</b> object from an upstream
	 * requirement.
	 * 
	 * @param target
	 *            The target is a {@link SpecialChapter}
	 * @param upstream
	 ...
	 * 
	 * @param target
	 *            The target is a {@link HierarchicalElement}
	 * @param upstream
	 *            The upstream requirement or null
	 * @param nextIndex
	 *            The next index to use to set the identifier feature of the
	 *            {@link CurrentRequirement}.
	 * @return the created current requirement
	 */
	public CurrentRequirement create(HierarchicalElement target,
			Requirement upstream, CompoundCommand compoundCmd) {
		long index = 0;
		HierarchicalElement root = RequirementHelper.INSTANCE
				.getHierarchicalElementRoot();
		String source = ""; //$NON-NLS-1$
		IRequirementCountingAlgorithm algorithm = RequirementCountingAlgorithmManager
				.getInstance().getCountingAlgorithm(
						ComputeRequirementIdentifier.getCurrentAlgorithm());
		...
		if (!compoundCmd.appendAndExecute(AddCommand.create(editingDomain, target,
				RequirementPackage.eINSTANCE
		...
		newCurrentReq.setIdentifier(ComputeRequirementIdentifier.INSTANCE
				.computeIdentifier(editingDomain, target, source, index));
	 ...
	 */
	private AnonymousRequirement createAnonymousRequirement(
			HierarchicalElement target) {
		AnonymousRequirement newAnonymous = RequirementFactory.eINSTANCE
				.createAnonymousRequirement();
		newAnonymous.setShortDescription(Messages
				.getString("RequirementHelper.0")); //$NON-NLS-1$
	 ...
	 */
	private CurrentRequirement createCurrentRequirementFromUpstream(
			Requirement upstream) {
		CurrentRequirement newCurrentReq = RequirementFactory.eINSTANCE
				.createCurrentRequirement();
	 ...
	 * 
	 * @param upstream
	 *            The source requirement that is an upstream requirement
	 * @param current
	 *            The target requirement that is a current requirement
	 */
	private void createDefaultAttributes(Requirement upstream,
			org.topcased.requirement.Requirement current) {
		AttributeConfiguration configuration = RequirementUtils
				.getAttributeConfiguration(editingDomain);
		for (ConfiguratedAttribute attribute : configuration
				.getListAttributes()) {
			...
			} else if (AttributesType.ALLOCATE_VALUE == attribute.getType()
					.getValue()) {
				newAttribute = createAttributeAllocate(attribute.getName());
			} else if (current instanceof CurrentRequirement
					&& AttributesType.LINK_VALUE == attribute.getType()
							.getValue()) {
				newAttribute = createAttributeLink(attribute.getName(),
						upstream);
			} else if (AttributesType.OBJECT_VALUE == attribute.getType()
					.getValue()) {
	...
	/**
	 * Creates a <b>{@link TextAttribute}</b> with the default values or with
	 * the value of the same attribute from the source requirement
	 * 
	 * @param attName
	 *            : the attribute name
	 * @param reqSource
	 ...
	 */
	private TextAttribute createAttributeText(ConfiguratedAttribute attribute,
			Requirement reqSource) {
		TextAttribute newAtt = RequirementFactory.eINSTANCE
				.createTextAttribute();
		newAtt.setName(attribute.getName());
		String value = attribute.getDefaultValue() != null ? attribute
				.getDefaultValue().getValue().getValue() : ""; //$NON-NLS-1$
		newAtt.setValue(value);
		return newAtt;
	}

	/**
	 * Creates the <b>{@link LinkAttributes}</b> corresponding to the upstream
	 * requirement dropped.
	 * 
	 * @param reqSource
	 *            The upstream requirement
	...
	/**
	 * Creates an <b>{@link AttributeLink}</b> object. The value is set to the
	 * reqSource requirement
	 * 
	 * @param name
	 *            The attribute name
	 * @param reqSource
	...
	/**
	 * Duplicates an existing <b>{@link AttributeLink} in taking into account
	 * only its name</b>.
	 * 
	 * @param attribute
	...
	public AttributeLink duplicateAttributeLink(AttributeLink attribute) {
		AttributeLink newOne = RequirementFactory.eINSTANCE
				.createAttributeLink();
	...
	private ObjectAttribute createAttributeReference(String name) {
		ObjectAttribute att = RequirementFactory.eINSTANCE
	...
	private AttributeAllocate createAttributeAllocate(String name) {
		AttributeAllocate att = RequirementFactory.eINSTANCE
				.createAttributeAllocate();
		att.setName(name);
		return att;
	}

	/**
	 * Duplicates an existing <b>{@link AttributeAllocate} in taking into
	 * account only its name</b>.
	 * 
	 * @param attribute
	 *            The attribute to duplicate
	 * @return the new duplicated attribute.
	 */
	public AttributeAllocate duplicateAttributeAllocate(
			AttributeAllocate attribute) {
		AttributeAllocate newOne = RequirementFactory.eINSTANCE
				.createAttributeAllocate();
		newOne.setName(attribute.getName());
		return newOne;
	}
	...
	/**
	 * Duplicates an existing <b>{@link ObjectAttribute} in taking into account
	 * only its name</b>.
	 * 
	 * @param attribute
	 *            The attribute to duplicate
	...
	public ObjectAttribute duplicateObjectAttribute(ObjectAttribute attribute) {
		ObjectAttribute newOne = RequirementFactory.eINSTANCE
				.createObjectAttribute();
		newOne.setName(attribute.getName());
		return newOne;
	}
	 ...
	 * @param element
	 *            The current requirement to rename
	 * @param nextIndex
	 *            The next index
	...
	public Command renameRequirement(CurrentRequirement requirement) {
		...
		if (requirement != null && algorithm != null) {
			...
			HierarchicalElement parent = (HierarchicalElement) requirement
					.eContainer();
			index = algorithm.getCurrentIndex(requirement);
			String newIdent = ComputeRequirementIdentifier.INSTANCE
					.computeIdentifier(editingDomain, parent, ident, index);
			algorithm.increaseIndexWhenCreateRequirement(requirement, index);
			return SetCommand.create(editingDomain, requirement,
					RequirementPackage.eINSTANCE
							.getIdentifiedElement_Identifier(), newIdent);
		}
		return UnexecutableCommand.INSTANCE;
	}

	/**
	 * Used by the default counting algorithm because the requirement index is
	 * stored on the hierarchical element root
	 * 
	 * @return the hierarchical element root of the current page model
	 */
	public HierarchicalElement getHierarchicalElementRoot() {
		if (currentPage != null
				&& currentPage.getModel() instanceof RequirementProject) {
			RequirementProject reqRoot = (RequirementProject) currentPage
					.getModel();
	...
	/**
	 * Moves up or moves down a <b>@link{CurrentRequirement}</b>. This command
	 * alters the order of the requirements into the list.
	 * 
	 * @param current
	 *            The current requirement to move
	 * @param up
	 *            If <b>true</b> move up, otherwise move down
	...
	public Command move(org.topcased.requirement.Requirement current, Boolean up) {
		if (current.eContainer() instanceof HierarchicalElement) {
			EList<org.topcased.requirement.Requirement> list = ((HierarchicalElement) current
					.eContainer()).getRequirement();
			return MoveCommand.create(editingDomain, current.eContainer(),
					RequirementPackage.eINSTANCE
							.getHierarchicalElement_Requirement(), current,
					list.lastIndexOf(current) + (up ? -1 : 1));
		} else if (current.eContainer() instanceof SpecialChapter) {
			EList<org.topcased.requirement.Requirement> list = ((SpecialChapter) current
					.eContainer()).getRequirement();
			return MoveCommand.create(editingDomain, current.eContainer(),
					RequirementPackage.eINSTANCE
							.getSpecialChapter_Requirement(), current,
					list.lastIndexOf(current) + (up ? -1 : 1));
		}
		return UnexecutableCommand.INSTANCE;
	}
	...
	/**
	 * Moves up or moves down a <b>@link{HierarchicalElement}</b>. This command
	 * alters the order of the hierarchical element into the list.
	 * 
	 * @param hierarchicalElt
	 *            The @link{HierarchicalElement} to move in its container
	 * @param up
	 *            If <b>true</b> move up, otherwise move down
	 * @return the EMF command which is of kind {@link MoveCommand}
	 */
	public Command move(HierarchicalElement hierarchicalElt, Boolean up) {
		...
		if (hierarchicalElt.eContainer() instanceof RequirementProject) {
			list = ((RequirementProject) hierarchicalElt.eContainer())
					.getHierarchicalElement();
			reference = RequirementPackage.eINSTANCE
					.getRequirementProject_HierarchicalElement();
		} else
		// default case
		{
			list = ((HierarchicalElement) hierarchicalElt.eContainer())
					.getChildren();
			reference = RequirementPackage.eINSTANCE
					.getHierarchicalElement_Children();

		}
		return MoveCommand.create(editingDomain, hierarchicalElt.eContainer(),
				reference, hierarchicalElt, list.lastIndexOf(hierarchicalElt)
						+ (up ? -1 : 1));
	}
	...
	public Command revertImpact(EObject requirement) {
		return SetCommand.create(editingDomain, requirement,
				RequirementPackage.eINSTANCE.getCurrentRequirement_Impacted(),
				false);
	}
	 ...
	 * 
	 * @param page
	 *            The content of the upstream view
	 */
	public void setUpstreamPage(UpstreamPage page) {
		if (page != null) {
			if (!page.equals(upstreamPage)) {
				...
				RequirementCoverageComputer.INSTANCE.reset(page
						.getEditingDomain());
			}
