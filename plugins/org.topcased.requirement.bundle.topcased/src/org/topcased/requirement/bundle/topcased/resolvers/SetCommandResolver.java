...
public class SetCommandResolver extends AdditionalCommand<SetCommand>
{
    ...
    private Map<SetCommand, Command> commands;

    public SetCommandResolver()
    {
        this(SetCommand.class);
        commands = new HashMap<SetCommand, Command>();
    }
    ...
    protected void pre_redo(List<SetCommand> setCommands)
    {
        for (SetCommand setCommand : setCommands)
        {
            Command compound = commands.get(setCommand);
            if (compound != null)
            {
                compound.redo();
            }
    ...
    protected void post_undo(List<SetCommand> setCommands)
    {
        for (ListIterator<SetCommand> i = setCommands.listIterator(setCommands.size()); i.hasPrevious();)
        {
            ...
            Command compound = commands.get(dndCommand);
            if (compound != null)
            {
                compound.undo();
            }
    ...
    protected List<Object> getSpecificCommands(Command command, Class< ? > clazz)
    {
        ...
        org.eclipse.emf.common.command.Command emfCmd = null;
        if (command instanceof org.topcased.modeler.commands.EMFtoGEFCommandWrapper)
        {
            emfCmd = ((org.topcased.modeler.commands.EMFtoGEFCommandWrapper) command).getEMFCommand();
        }
        if (command instanceof EMFtoGEFCommandWrapper)
        {
            emfCmd = ((EMFtoGEFCommandWrapper) command).getEMFCommand();
        }
        if (emfCmd instanceof CompoundCommand)
        {
            CompoundCommand compound = (CompoundCommand) emfCmd;
