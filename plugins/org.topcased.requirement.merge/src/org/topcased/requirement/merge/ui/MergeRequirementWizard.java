			...
			getContainer().run(true, true, new IRunnableWithProgress() {
	            ...
	            InterruptedException {
					try {
						...
						Merge m = new Merge(page.getInputs(), page
								.getOutputValue());
						m.process(monitor);
					} catch (Throwable e) {
	...
	public void init(ISelection selection1) {
		...
		if (selection1 instanceof IStructuredSelection) {
			...
			for (Iterator<?> iterator = selection.iterator(); iterator
				...
				if (f != null
						&& f.getLocation().getFileExtension().toLowerCase()
								.endsWith("di")) {
					inputs.add(URI.createPlatformResourceURI(
							f.getFullPath().toString(), true).toString());
				}
			}
